...
def crop_image(image, start_row, start_col, num_rows, num_cols):
    ...
    num_cols=num_cols+start_col
    return image[start_row:num_rows,start_col:num_cols]
    ### END YOUR CODE
...
def resize_image(input_image, output_rows, output_cols):
    ...
    cols_scale_factor=input_cols/output_cols
    for i in range (output_rows):
                for j in range (output_cols):
                    input_i=int(i*row_scale_factor)
                    input_j=int(j*cols_scale_factor)
...
def rotate2d(point, theta):
    ...
    rotated_point=np.matmul(rotation, point)
...
def rotate_image(input_image, theta):
    ...
    for i in range (input_rows):
        for j in range(input_cols):
             x_centered=j-center_x
             y_centered=i-center_y

             centered_point=np.array([x_centered,y_centered])

             rotated_point=rotate2d(centered_point,-theta)
